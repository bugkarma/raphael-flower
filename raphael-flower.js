      ...
      _drawFlower = function(options) {
        ...
        // Get SVG path string for a petal
          getPetalPathString = function(i) {
            var path  = 'M ' + (_origin.x + (cr * Math.cos(2 * Math.PI * i-0 / np))) + ' ';
                path += (_origin.y + (cr * Math.sin(2 * Math.PI * i-0 / np)));
        ...
        for (;i<np;i++) {
          ...
          newAngle = _getRandom(newAngle-as.max, newAngle+as.max);                                    // Apply swing
          
          if (newAngle>=360)  newAngle -= 360;                                                        // Keep angle 0 <= n > 360
          if (newAngle<0)     newAngle += 360;
          
          newOriginString = _origin.x + ' ' + _origin.y;                                              // Stringify origin
